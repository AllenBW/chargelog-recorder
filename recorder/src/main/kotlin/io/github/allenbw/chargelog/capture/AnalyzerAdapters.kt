 ...
 * (see `measure/AnalyzerSample.kt`'s `SampleEntity.toAnalyzerSample()`): both fold `scale` into
 * the level ([Units.levelPct]) and drop it, since `AnalyzerSample` has no slot for it, and both
 * map `t -> wallClockMs` / `e -> elapsedRealtimeMs`.
